...
class Dat(BaseDat):
    ...
    def copy(self, other, subset=None):
        ...
        if subset is None:
            ...
            if self.halo_valid:
                if opencl_backend.offloading:
                    other._opencl_data[:] = self.data_ro
                else:
                    other._data[:] = self.data_ro

                if other.can_be_represented_as_petscvec:
                    other._vec.setOffloadMask(self._vec.getOffloadMask())
                else:
                    other._availability_flag = self._availability_flag
...
class Global(BaseGlobal):
    ...
    def _vec(self):
        ...
        if self.comm.rank == 0:
            ...
            return PETSc.Vec().createViennaCLWithArrays(cla.empty(opencl_backend.queue, shape=0, dtype=self.dtype),
                                                        size=size,
                                                        bsize=self.cdim,
                                                        comm=self.comm)
...
class Constant(BaseConstant):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._opencl_data = cla.to_device(opencl_backend.queue, self._data)

    def get_availability(self):
        return AVAILABLE_ON_BOTH
    ...
    def data_ro(self):
        ...
        if opencl_backend.offloading:
            view = self._opencl_data.view()
            # FIXME: PyOpenCL doesn't support read-only arrays yet
            return view
        else:
            view = self._data.view()
            view.setflags(write=False)

...
class Global(BaseGlobal):
    ...
    def _vec(self):
        ...
        if self.comm.rank == 0:
            ...
            return PETSc.Vec().createCUDAWithArrays(cuda_np.empty(0, dtype=self.dtype),
                                                    size=size,
                                                    bsize=self.cdim,
                                                    comm=self.comm)
...
class Constant(BaseConstant):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._cuda_data = cuda_np.to_gpu(self._data)
    ...
    def __call__(self, grid, block, *args):
        if all(grid + block):
            self.cu_func.prepared_call(grid, block,
                                       *[arg.gpudata
                                         if isinstance(arg, cuda_np.GPUArray)
                                         else arg
                                         for arg in args],
